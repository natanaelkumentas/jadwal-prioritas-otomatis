...
export default function ShiftCodeModal({ onClose }: ShiftCodeModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-955/80 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-slate-900 border border-slate-700 rounded-t-2xl sm:rounded-xl max-w-2xl w-full p-4 sm:p-6 shadow-2xl flex flex-col max-h-[85vh] animate-slide-in-right z-10 safe-area-bottom">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-200 p-1.5 rounded-lg bg-slate-800 border border-slate-700 transition-colors"
            title="Tutup"
          >
        ...
        <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 text-xs">
          {shiftDefinitions.map(def => (
            <div
              key={def.code}
              className="p-3 bg-slate-950/80 border border-slate-800/80 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-2 hover:border-slate-700 transition-colors"
            >
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <h4 className="font-bold text-slate-200 text-xs sm:text-sm">{def.title}</h4>
                  <p className="text-[11px] text-slate-400 mt-0.5 leading-relaxed">{def.description}</p>
                </div>
              </div>

              <div className="flex items-center gap-3 text-[10px] sm:text-xs text-slate-400 bg-slate-900 px-2.5 py-1.5 rounded border border-slate-800 self-start sm:self-auto whitespace-nowrap">
                <FiClock className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                <div className="flex flex-col">
                  <span>CNS: <strong className="text-slate-200">{def.cnsTime}</strong></span>
                  <span>ESS: <strong className="text-slate-200">{def.essTime}</strong></span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="pt-3 mt-3 border-t border-slate-800 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold rounded-lg text-xs border border-slate-700 transition-colors"
          >
            Tutup
          </button>
        </div>
